      ...
      <div className="mt-[28px] flex flex-col items-center gap-[28px] max-lg:gap-[12px] max-md:gap-[12px]">
        {t.advantages.items.map((item, i) => {
          ...
          return (
            ...
            >
              {/* Background band — always present, opacity transitions */}
              <div
                className="absolute inset-0 bg-[#4F5E4A] mix-blend-multiply transition-opacity duration-300"
                style={{ opacity: isActive ? 0.15 : 0 }}
              />

              {/* Title — always in the same position */}
              <p
                className="relative pt-[16px] text-center font-[family-name:var(--font-roboto)] text-[clamp(4rem,6.67vw,8rem)] font-black uppercase leading-[0.85] tracking-[-0.02em] text-[#4F5E4A] transition-opacity duration-300 max-lg:pt-[7px] max-lg:text-[64px] max-lg:tracking-[-1.28px] max-md:pt-[7px] max-md:text-[32px] max-md:tracking-[-0.64px]"
                style={{ opacity: isActive ? 1 : 0.55 }}
              >
                {item.title}
              </p>

              {/* Expandable description — grid row transition */}
              <div
